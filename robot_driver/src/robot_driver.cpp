...
RobotDriver::RobotDriver(ros::NodeHandle nh, int argc, char **argv)
{
  ...
  if (is_hardware_)
  {
    ROS_INFO_STREAM("Robot type id: " << robot_name);
    //if (robot_name == "go1") { // TODO FIXME
...
bool RobotDriver::updateControl()
{
  ...
  if (control_mode_ == SAFETY)// mod from security
  {
  ...
  else if (control_mode_ == READY)// ready to leg control
  {
...
void RobotDriver::publishControl(bool is_valid)
{
  ...
  if (is_hardware_ && is_valid)// go to real robot
  {
